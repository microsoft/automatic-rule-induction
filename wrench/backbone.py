...
class BertTextClassifier_VAT(BERTBackBone):
    """
    Bert with a MLP on top for text classification
    """

    def __init__(self, n_class, model_name='bert-base-cased', fine_tune_layers=-1, max_tokens=512, binary_mode=False, **kwargs):
        super(BertTextClassifier_VAT, self).__init__(n_class=n_class, model_name=model_name, fine_tune_layers=fine_tune_layers, binary_mode=binary_mode)
    ...
    def forward(self, batch, return_features=False, VAT=False):  # inputs: [batch, t]
        device = self.get_device()
        outputs = self.model(input_ids=batch["input_ids"].to(device), attention_mask=batch['mask'].to(device))
        h = self.dropout(outputs.pooler_output)

        if VAT:
            vat = self.vat_loss(self.classifier, h)
        else:
            vat = None
